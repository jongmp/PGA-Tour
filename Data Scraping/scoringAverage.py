...
rows = []
rows.append(['Player Name', 'Rounds', 'Average Score', 'Total Strokes','Total Adjustment','Total Rounds', 'Year'])
#print(rows)
...
for i in year_list:
    ...
    urlpage = 'https://www.pgatour.com/stats/stat.120.' + i + '.html'
    ...
    for result in results:
        ...
        total_stroke = data[5].getText()
        total_adj = data[6].getText()
        total_round = data[7].getText()
        ...
        rows.append([name, rounds, average, total_stroke, total_adj, total_round, year])

    print(rows)

# Create a csv file and write rows to the file
with open('scoringAverage.csv', 'w', newline='') as f_output:

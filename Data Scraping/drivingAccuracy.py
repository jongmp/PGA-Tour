...
rows = []
rows.append(['Player Name', 'Rounds', 'Fairway Percentage', 'Fairways Hit','Possible Fairways', 'Year'])
#print(rows)
...
for i in year_list:
    ...
    urlpage = 'https://www.pgatour.com/stats/stat.102.' + i + '.html'
    ...
    for result in results:
        ...
        fairways_hit = data[5].getText()
        possible_fairways = data[6].getText()
        ...
        rows.append([name, rounds, percentage, fairways_hit, possible_fairways, year])

    print(rows)

# Create a csv file and write rows to the file
with open('drivingAccuracy.csv', 'w', newline='') as f_output:

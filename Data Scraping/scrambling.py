...
rows = []
rows.append(['Player Name', 'Rounds', 'Average Scrambling', 'Par or Better','Missed GIR', 'Year'])
#print(rows)
...
for i in year_list:
    ...
    urlpage = 'https://www.pgatour.com/stats/stat.130.' + i + '.html'
    ...
    for result in results:
        ...
        par_better = data[5].getText()
        missedGIR = data[6].getText()
        ...
        rows.append([name, rounds, percentage, par_better, missedGIR, year])

    print(rows)

# Create a csv file and write rows to the file
with open('scrambling.csv', 'w', newline='') as f_output:

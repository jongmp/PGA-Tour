...
rows = []
rows.append(['Player Name', 'Rounds', 'Green In Regulation Percentage', 'Greens Hit','Holes','Relative/Par', 'Year'])
#print(rows)
...
for i in year_list:
    ...
    urlpage = 'https://www.pgatour.com/stats/stat.103.' + i + '.html'
    ...
    for result in results:
        ...
        percentage = data[4].getText()
        greens_hit = data[5].getText()
        holes = data[6].getText()
        relative = data[7].getText()
        ...
        rows.append([name, rounds, percentage, greens_hit, holes, relative, year])

    print(rows)

# Create a csv file and write rows to the file
with open('greenInRegulation.csv', 'w', newline='') as f_output:

...
rows = []
rows.append(['Player Name', 'Rounds', 'Average Putts', 'Total Putts','Total Rounds','Low Total Putts','Year'])
#print(rows)
...
for i in year_list:
    ...
    urlpage = 'https://www.pgatour.com/stats/stat.119.' + i + '.html'
    ...
    for result in results:
        ...
        total_putt = data[5].getText()
        total_round = data[6].getText()
        low_total = data[7].getText()
        ...
        rows.append([name, rounds, average, total_putt, total_round, low_total, year])

    print(rows)

# Create a csv file and write rows to the file
with open('puttPerRound.csv', 'w', newline='') as f_output:

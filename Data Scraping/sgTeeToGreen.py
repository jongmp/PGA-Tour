...
rows = []
rows.append(['Player Name', 'Rounds','Average SG', 'SG:OTT', 'SG:APR','SG:ARG','MEASURED ROUNDS','Year'])
#print(rows)
...
for i in year_list:
    ...
    urlpage = 'https://www.pgatour.com/stats/stat.02674.' + i + '.html'
    ...
    for result in results:
        ...
        sg_ott = data[5].getText()
        sg_apr = data[6].getText()
        sg_agr = data[7].getText()
        ...
        rows.append([name, rounds, average, sg_ott, sg_apr, sg_agr, measured_round, year])

    print(rows)

# Create a csv file and write rows to the file
with open('sgTeeToGreen.csv', 'w', newline='') as f_output:
